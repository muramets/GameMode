...
class Storage {
  ...
  addInnerface(innerfaceData) {
    ...
    const newInnerface = {
      ...
      color: innerfaceData.color || null, // Не задаем дефолтный цвет
  ...
  updateInnerfaceFull(innerfaceId, innerfaceData) {
    ...
    innerfaces[index] = {
      ...
      hover: innerfaceData.hover || '',
      initialScore: innerfaceData.initialScore,
      color: innerfaceData.color || innerfaces[index].color || null, // Сохраняем существующий или null
  ...
  mergeDataArrays(localArray, serverArray, dataType) {
    ...
    if (dataType === 'history') {
      ...
      console.log("🔄 USING TIMESTAMP-BASED STRATEGY FOR NON-HISTORY DATA");
