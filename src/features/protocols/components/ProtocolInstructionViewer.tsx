...
export const ProtocolInstructionViewer = React.memo(({ instruction, isExpanded }: ProtocolInstructionViewerProps) => {
    return (
        <AnimatePresence>
            {isExpanded && instruction && (
                <motion.div
                    ...
                    className="overflow-hidden border-t border-sub/10 bg-sub-alt/50 px-4"
                >
                    <div className="py-4 prose prose-invert prose-sm max-w-none text-sub font-mono text-xs leading-relaxed select-text cursor-text" onClick={(e) => e.stopPropagation()}>
                        <ReactMarkdown
                            components={{
