...
/**
 * Resolves a color value to a hex string.
 * - If color is a CSS variable (e.g., 'var(--main-color)'), resolves it to hex.
 * - If color is already hex, returns it as-is.
 * - Falls back to provided fallback or '#ffffff' if resolution fails.
 */
export function resolveColor(color: string | undefined, fallback: string = '#ffffff'): string {
    if (!color) return fallback;

    // If already hex, return as-is
    if (color.startsWith('#')) return color;

    // If CSS variable, resolve it
    if (color.startsWith('var(')) {
        ...
        if (match) {
            ...
            if (typeof document !== 'undefined') {
                ...
                if (computedValue) {
                    // If computed value is rgb(...), convert to hex
                    ...
                    // Try parsing as space-separated RGB (e.g., "226 183 20")
                    const parts = computedValue.split(/\s+/).map(Number);
...
/**
 * Converts an rgb/rgba string to hex.
 */
