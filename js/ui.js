...
const UI = {
  ...
  emojiToFontAwesome(emoji) {
    const emojiMap = {
      ...
      // === Custom User Requested Mappings ===
      '🏊': 'fas fa-swimmer',          // Water volleyball / Swimming
      '🏋': 'fas fa-dumbbell',         // Weight training / Workouts
      '🎥': 'fas fa-video',            // YouTube / Video content
      '💵': 'fas fa-dollar-sign',      // Money / Finance
      '🎪': 'fas fa-gamepad',          // Entertainment / Fun
      '🔒': 'fas fa-lock',             // Discipline / Security
      '🏦': 'fas fa-briefcase',        // Entrepreneur Mode / Business
      '🩷': 'fas fa-heart'             // Relationship / Love
    };
  ...
  renderHistory() {
    ...
    container.innerHTML = App.filteredHistory.map(checkin => {
      ...
      if (checkin.type === 'drag_drop') {
        ...
        // 🔧 НОВОЕ: Формируем описание с учетом фильтра
        let actionDesc = `Position: ${checkin.oldPosition} → ${checkin.newPosition}`;
        if (checkin.filterInfo && checkin.filterInfo.isFilterSpecific && checkin.filterInfo.filterName) {
          actionDesc += ` for ${checkin.filterInfo.filterName} view`;
        }
